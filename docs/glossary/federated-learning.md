...
一种"数据不动模型动"的学习方式。多个参与方不用把数据集中到一起，而是在各自本地训练模型，只把学到的"经验"汇总起来。就像大家各自在家做题，只交流解题方法，不用把试卷给别人看，既学到了东西又保护了隐私。
...
- $w$：全局模型参数
- $K$：客户端总数
- $p_k$：客户端 $k$ 的权重（通常按数据量比例分配）
- $F_k(w)$：客户端 $k$ 的本地目标函数

### 为什么联邦学习有效

1. **数据分布多样性**：各客户端的数据分布不同，联合建模可以覆盖更全面的样本空间
2. **隐私保护**：原始数据不出本地，仅交换模型参数或梯度，降低数据泄露风险
3. **通信效率**：相比传输原始数据，传输模型更新的通信量更小
4. **合规性**：满足 GDPR、《个人信息保护法》等数据保护法规的要求

### 联邦平均（FedAvg）的收敛性

FedAvg 的核心思想是：各客户端在本地进行多轮 SGD（Stochastic Gradient Descent）更新，然后服务器对模型进行加权平均。

**收敛条件**：

- 目标函数为凸函数时，FedAvg 收敛速率为 $O(1/T)$（T 为通信轮次）
- 非凸函数时，收敛到驻点（Stationary Point）
- Non-IID 数据会影响收敛速度和最终精度

### 隐私保护的数学基础

- **差分隐私**（Differential Privacy）：通过添加噪声，使得单个样本的存在与否对输出影响有限
- **安全多方计算**（Secure Multi-Party Computation）：多方协作计算函数，不暴露各自的输入
- **同态加密**（Homomorphic Encryption）：在加密数据上直接进行计算，解密后结果与明文计算一致
