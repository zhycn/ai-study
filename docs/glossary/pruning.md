...
剪枝的理论基础是**神经网络的过参数化（Over-parameterization）**。现代深度学习模型通常包含远超任务所需的参数量，这为剪枝提供了空间。

### 稀疏性与泛化

研究表明，神经网络中存在一个**稀疏子网络（Sparse Subnetwork）**，仅用少量参数就能达到与完整模型相近的性能。这就是著名的**幸运彩票假说（Lottery Ticket Hypothesis）**：

> 在一个随机初始化的密集网络中，存在一个子网络，当独立训练时，能在相同迭代次数内达到与原网络相当的测试精度。
...
由于 L0 约束是非凸且 NP-Hard 的，实际中通常用以下方法近似：
- **L1 正则化**：诱导稀疏性
- **幅度阈值**：直接移除小权重
- **重要性评分**：基于梯度或 Hessian 矩阵

### 剪枝的三个阶段

| 阶段 | 操作 | 目的 |
|------|------|------|
| 剪枝（Prune） | 根据重要性评分移除参数 | 减少参数量 |
| 微调（Fine-tune） | 在训练数据上继续训练 | 恢复精度 |
| 迭代（Iterate） | 重复剪枝-微调循环 | 逐步提高稀疏度 |
...
```text
稀疏度 0-50%：  精度几乎无损（安全区）
稀疏度 50-70%： 精度轻微下降（可接受区）
稀疏度 70-90%： 精度明显下降（需微调恢复）
稀疏度 90%+：   精度严重损失（通常不可用）
```

:::info 信息
不同模型对剪枝的敏感度不同。CNN 模型通常可以剪枝 70-90% 而精度损失很小，而 LLM 由于自注意力机制的复杂性，安全剪枝比例通常在 20-50%。
:::
...
不同剪枝策略在稀疏度、加速效果和实现难度上各有侧重：

| 方案 | 剪枝粒度 | 稀疏度 | 硬件加速 | 精度损失 | 实现难度 |
|------|----------|--------|----------|----------|----------|
| 幅度剪枝（Magnitude） | 单个权重 | 高（90%+） | 需稀疏库 | 低 | 低 |
| 结构化剪枝 | 通道/头/层 | 中（30-70%） | 直接加速 | 中 | 中 |
| LLM-Pruner | 注意力头/FFN | 中（20-50%） | 直接加速 | 低 | 高 |
| ShortGPT | Transformer 层 | 低（10-30%） | 直接加速 | 低 | 中 |
| Sheared LLaMA | 混合粒度 | 高（70%+） | 直接加速 | 低 | 高 |
| Wanda | 单个权重 | 高（50-75%） | 需稀疏库 | 低 | 低 |

:::tip 选择建议
- **追求推理加速**：选择结构化剪枝，可直接在标准硬件上加速
- **追求极致压缩**：选择非结构化剪枝 + 稀疏计算库
- **LLM 剪枝**：选择 LLM-Pruner 或 Sheared LLaMA，专为大模型设计
- **快速基线**：选择 Wanda（无需微调）或幅度剪枝，实现简单
:::
