...
给 AI 装上"手脚"和"记忆"，让它不仅能聊天，还能真正帮你干活。普通的 AI 只能回答问题，Agent 则能自己搜索网页、操作软件、调用工具，像一个小助手一样独立完成任务。
...
agent:
  ...
  description: 处理客户咨询和投诉的智能客服 Agent
  capabilities:
    - 查询订单状态
    - 处理退换货申请
    - 回答常见问题
...
| 场景          | 推荐框架          | 架构模式            |
| ------------- | ----------------- | ------------------- |
| 简单工具调用  | OpenAI Agents SDK | 单 Agent + 工具     |
| 复杂工作流    | LangGraph         | 图结构工作流        |
| 多 Agent 协作 | AutoGen / CrewAI  | 多 Agent 对话       |
| 企业级应用    | LangGraph + MCP   | 图结构 + 标准化工具 |
...
def planner(state: AgentState) -> AgentState:
    ...
    # 调用 LLM 分析当前状态，决定下一步
    ...
    return {'current_step': 'tool_execution'}

def tool_executor(state: AgentState) -> AgentState:
    """执行工具调用"""
    # 根据 current_step 调用对应工具
    ...
...
graph.add_node('responder', response_generator)
...
def execute_tool(tool_name: str, params: dict) -> str:
    if tool_name == 'order_query':
        return query_order_api(params['order_id'])
    elif tool_name == 'faq_search':
        return search_faq(params['query'])
    else:
...
# 测试用例
test_cases = [
    {'input': '我的订单 12345 到哪了？', 'expected_tool': 'order_query'},
    {'input': '怎么退货？', 'expected_tool': 'faq_search'},
    {'input': '我要退款', 'expected_action': 'human_handoff'},  # 需人工介入
]
...
- **可观测性**：记录每次执行的 Thought-Action-Observation 链路
- **性能监控**：跟踪响应时间、工具调用成功率、错误率
- **安全审计**：记录所有工具调用，定期审查异常行为
- **持续优化**：根据用户反馈调整提示词和工具定义

## 最佳实践

### Agent 设计原则

- **单一职责**：每个 Agent 专注于特定领域，避免"万能 Agent"
- **明确边界**：清晰定义能力范围，超出范围时优雅降级或转人工
- **可观测性**：完整记录决策链路，便于调试和审计
- **安全优先**：工具调用前验证权限，敏感操作需人工确认
- **幂等设计**：同一输入多次执行应产生一致结果

### 调试技巧

- **思维可视化**：输出完整的 Thought-Action-Observation 序列
- **工具调用日志**：记录每次调用的参数、返回值、耗时
- **断点调试**：在关键节点暂停，检查状态是否符合预期
- **回放功能**：重现历史执行过程，定位问题根因

### 多 Agent 协作模式

| 模式     | 说明                                       | 适用场景           |
| -------- | ------------------------------------------ | ------------------ |
| 顺序协作 | Agent A 输出作为 Agent B 输入              | 流水线式任务       |
| 并行协作 | 多个 Agent 同时处理不同子任务              | 独立子任务         |
| 投票决策 | 多个 Agent 独立判断，投票决定              | 需要高可靠性的决策 |
| 层级管理 | 一个 Manager Agent 分配任务给 Worker Agent | 复杂任务分解       |
