...
用"点连线"的方式把知识组织起来的数据库。每个"点"代表一个事物（比如"马云"），每条"线"代表它们之间的关系（比如"马云-创办了-阿里巴巴"）。电脑有了这张网，就能理解事物之间的关联，做出更有逻辑的推理。
...
| 场景 | 推荐方案 | 理由 |
| ---- | -------- | ---- |
| 快速原型 | Neo4j 社区版 | 生态成熟，文档丰富，Cypher 语法直观 |
| 大规模生产 | NebulaGraph | 分布式架构，支持超大规模图数据 |
| 灵活部署 | JanusGraph | 后端可插拔，支持多种存储引擎 |
| 实时图分析 | TigerGraph | 原生并行计算，GSQL 查询语言强大 |
| RDF 标准兼容 | Virtuoso | W3C 标准支持，SPARQL 查询 |

### 关键决策因素

- **数据规模**：千万级实体以下选 Neo4j，亿级以上选 NebulaGraph
- **查询模式**：多跳查询频繁选原生图数据库，简单关系可选 PG 扩展
- **团队技能**：熟悉 Java 考虑 Neo4j/JanusGraph，熟悉 C++ 考虑 NebulaGraph
- **生态需求**：需要丰富工具链选 Neo4j，需要国产化选 NebulaGraph
...
| ---- | -------- | ---------- |
| 数据结构 | 图结构（实体 + 关系） | 高维向量 |
| 查询方式 | 精确关系查询、多跳推理 | 语义相似度检索 |
| 可解释性 | 高，推理路径可追溯 | 低，黑盒相似度 |
| 构建成本 | 高，需要信息抽取和融合 | 低，Embedding 模型自动生成 |
| 适用场景 | 关系推理、事实查询 | 语义搜索、推荐系统 |

:::tip 选型原则
知识图谱和向量数据库不是互斥的。GraphRAG 架构将两者结合，用知识图谱提供精确关系推理，用向量数据库提供语义相似度检索，效果最佳。
:::
