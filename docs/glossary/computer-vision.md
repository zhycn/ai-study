...
计算机视觉的本质是将像素矩阵转化为语义理解，通过多层次的特征提取和抽象，建立从底层视觉信号到高层语义概念的映射。

### 视觉信息的层次化处理

人类视觉系统和计算机视觉都遵循从简单到复杂的层次化处理：

- **底层视觉**（Low-level Vision）：边缘检测、角点提取、纹理分析。处理局部像素模式
- **中层视觉**（Mid-level Vision）：形状识别、目标分割、运动估计。组合底层特征形成结构化表示
- **高层视觉**（High-level Vision）：目标识别、场景理解、行为分析。结合上下文进行语义推理

### 卷积的核心思想

**卷积**（Convolution）是 CNN 的数学基础，具有三个关键性质：

- **局部连接**（Local Connectivity）：每个神经元只感受输入的一小块区域，符合视觉皮层的感受野特性
- **权重共享**（Weight Sharing）：同一卷积核在整张图像上滑动，大幅减少参数量
- **平移不变性**（Translation Invariance）：无论目标出现在图像的哪个位置，都能被检测到

### 特征学习 vs 手工特征

| 方式 | 描述 | 代表方法 | 优缺点 |
|------|------|----------|--------|
| **手工特征** | 专家设计特征提取规则 | SIFT、HOG、LBP | 可解释性强，但泛化能力有限 |
| **特征学习** | 模型从数据中自动学习特征 | CNN、ViT | 泛化能力强，但需要大量数据和算力 |

### 感受野与层次化抽象

随着网络层数加深，感受野（Receptive Field）逐层扩大：
...
这种层次化抽象使网络能够从像素级特征逐步构建语义级表示，是深度视觉模型成功的核心原因。

### 语义鸿沟（Semantic Gap）

计算机视觉的核心挑战在于**语义鸿沟**：底层像素数据与高层语义理解之间存在巨大差距。同一物体在不同光照、角度、遮挡条件下，像素值可能完全不同。深度学习通过端到端的特征学习，在一定程度上缩小了这一鸿沟。
