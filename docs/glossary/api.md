...
| 场景 | 推荐方案 | 理由 |
| ---- | -------- | ---- |
| 通用对话 | OpenAI GPT-4o | 生态最完善，工具链丰富 |
| 长文本处理 | Claude 3.5/4、Kimi | 长上下文支持优秀 |
| 多模态理解 | Gemini 2.0、GPT-4 Vision | 原生多模态架构 |
| 国产合规 | 智谱 GLM-4、文心一言 | 数据不出境，符合监管 |
| 成本控制 | GPT-4o-mini、开源模型 API | 性价比高，适合大批量调用 |
| 多模型路由 | LiteLLM、One API | 统一接口，便于切换和对比 |

### 关键决策因素

- **任务类型**：对话选 GPT-4o/Claude，代码选 Claude/GPT-4，多模态选 Gemini
- **成本预算**：高频调用选 mini 模型或开源方案，低频调用选旗舰模型
- **数据合规**：国内业务优先国产模型，海外业务可选国际厂商
- **延迟要求**：实时交互选流式输出，批量处理可接受同步响应

### API 调用 vs 本地部署

| 维度 | API 调用 | 本地部署 |
| ---- | -------- | -------- |
| 初始成本 | 零，按量付费 | 高，需购买 GPU |
| 运维复杂度 | 低，服务商负责 | 高，需自行维护 |
| 数据隐私 | 数据发送至第三方 | 数据完全可控 |
| 定制能力 | 有限，仅 Prompt/微调 | 完全可控 |
| 延迟 | 受网络影响 | 本地网络，延迟低 |
| 适用场景 | 快速验证、中小规模 | 大规模、高隐私要求 |

:::tip 选型原则
项目初期优先使用 API 快速验证，确认需求和规模后再评估是否需要本地部署。使用 LiteLLM 等网关层抽象，便于后续切换提供商。
:::
