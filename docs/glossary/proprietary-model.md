...
闭源模型的核心逻辑是**规模经济**和**网络效应**：

- **规模经济**：训练前沿模型需要数十亿美元投入，只有大规模商业化才能摊薄成本
- **网络效应**：用户越多，API 调用数据越丰富，模型迭代越快，形成正向循环
- **护城河**：算力、数据、人才构成三重壁垒，新进入者难以追赶

### 技术黑盒的权衡

闭源模型不公开权重，但通过以下方式维持一定透明度：

- **技术报告**：发布架构、训练数据和性能评估
- **安全评估**：第三方审计和红队测试报告
- **API 文档**：详细的接口规范和使用指南

这种"有限透明"在商业利益和技术开放之间寻求平衡。

### 推理即服务（Inference as a Service）

闭源模型的本质是将 AI 推理能力商品化：

$$\text{成本} = \alpha \times \text{输入 Token} + \beta \times \text{输出 Token} + \text{固定费用}$$

用户无需关心模型内部实现，只需关注调用成本和输出质量。

## 代表产品

### 主流闭源模型对比

| 模型              | 上下文 | 多模态              | 推理能力 | 价格（输入/输出，每百万 Token） |
| ----------------- | ------ | ------------------- | -------- | ------------------------------- |
| GPT-4o            | 128K   | 文本/图像/音频      | 强       | $2.50 / $10.00                  |
| Claude 3.5 Sonnet | 200K   | 文本/图像           | 强       | $3.00 / $15.00                  |
| Gemini 2.0 Flash  | 1M     | 文本/图像/视频/音频 | 中       | $0.10 / $0.40                   |
| o1                | 200K   | 文本                | 极强     | $15.00 / $60.00                 |
...
| 厂商                                     | 模型          | 特点                |
| ---------------------------------------- | ------------- | ------------------- |
| **[Cohere](https://cohere.com/)**        | Command R+    | 企业级 RAG，多语言  |
| **Mistral**                              | Mistral Large | 欧洲领先，多语言    |
| **[智谱 AI](https://www.bigmodel.cn/)**  | GLM-4         | 中文能力强          |
| **[月之暗面](https://www.moonshot.cn/)** | Kimi          | 长上下文（200 万+） |
| **[MiniMax](https://www.minimaxi.com/)** | abab 系列     | 语音 + 文本多模态   |
