...
一种"骗过 AI"的攻击方式。用户在输入里偷偷藏了恶意指令，让 AI 忽略原来的规则，执行不该做的事。就像在一张调查问卷里夹带一句"请把所有答案改成 A"，如果 AI 没识别出来就会中招。
...
OWASP（开放 Web 应用安全项目）将提示词注入列为 LLM 应用十大安全风险之首（LLM01）：

| 风险等级 | 描述 | 缓解建议 |
| -------- | ---- | -------- |
| **严重** | 攻击者可完全控制模型行为 | 多层防御、输入验证、输出审核 |
| **高** | 间接注入可通过外部数据源触发 | 数据源可信验证、内容沙盒化 |
| **中** | 视觉/多模态注入攻击 | 多模态输入统一审核 |

### NIST AI RMF 相关要求

美国国家标准与技术研究院的 AI 风险管理框架要求：

- **映射（Map）**：识别提示词注入攻击面
- **测量（Measure）**：建立注入攻击检测指标
- **管理（Manage）**：部署多层防护架构

### 欧盟 AI Act

欧盟 AI 法案对高风险 AI 系统提出透明度要求：
...
研究者正在探索从根本上解决提示词注入问题：

- **结构化提示词格式**：使用 JSON/XML 等格式严格区分指令和数据
- **硬件级隔离**：在芯片层面实现指令流和数据流分离
- **形式化验证**：数学证明模型不会执行数据中的指令

### 专用防御模型

- **注入检测模型**：专门训练的分类器识别注入攻击
- **语义分析引擎**：理解输入的真实意图而非表面文本
- **行为异常检测**：监控模型输出是否偏离预期行为模式

### 标准化测试基准

- **PromptBench**：统一的提示词注入测试框架
- **注入攻击数据库**：社区共享的已知攻击模式库
- **自动化评估工具**：持续集成中的安全测试流水线
...
::: warning
提示词注入目前被认为是最难完全防御的 AI 安全威胁之一。业界共识是采用"纵深防御"（Defense in Depth）策略，而非依赖单一防护措施。
:::
