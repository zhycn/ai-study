...
AI 读文字不是按"字"读的，而是把文字切成一个个小片段，每个片段就叫一个 Token。就像切蛋糕一样，把一段话切成一块一块的，AI 才能消化理解。
...
不同语言在 Token 化时的表现差异显著：

| 语言     | 示例           | Token 数 | 说明                     |
| -------- | -------------- | -------- | ------------------------ |
| 英文     | Hello world    | 2        | 空格分隔，词边界清晰     |
| 中文     | 你好世界       | 4        | 通常一字一 Token         |
| 日文     | こんにちは     | 3-6      | 取决于 Tokenizer 词表    |
| 阿拉伯文 | مرحبا بالعالم  | 4-8      | 连写字符增加 Token 化难度 |
| 代码     | `def func():`  | 5-7      | 符号和关键字分别 Token 化 |
...
# 批量计算
def count_batch_tokens(messages: list[dict], model: str = "gpt-4") -> int:
    """计算对话消息的总 Token 数"""
    enc = tiktoken.encoding_for_model(model)
    # 每条消息有固定开销
...
| 去除冗余空格     | 规范化空白字符                 | 5-10%       |
| 使用缩写         | 长词替换为短词                 | 10-20%      |
| 结构化提示       | 用 JSON/YAML 替代自然语言描述  | 20-30%      |
| 函数调用         | 用 structured output 替代长提示 | 30-50%      |
...
TOKENIZER_MAP = {
    ...
    "claude-3": "cl100k_base",  # Claude 使用类似词表
    "llama-3": "tiktoken",      # LLaMA 3 使用 tiktoken
    "mistral": "sentencepiece", # Mistral 使用 SentencePiece
}
...
::: tip
始终使用与目标模型匹配的 Tokenizer 进行 Token 计数。使用错误的 Tokenizer 会导致 10-30% 的计数误差。
:::
